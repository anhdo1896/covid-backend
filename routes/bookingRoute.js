  ...
  try {
    ...
    bookings.forEach(async (booking) => {
      ...
      if (newStatus == 0) {
        ...
        Hospital.findByIdAndUpdate(booking.hospitalId, {
          freeBeds: myFreeBeds,
        });
      }
  ...
  axios
    ...
    .then(function (response) {
      ...
      response["data"]["data"].forEach((day, i) => {
        let obj = [
          moment(day.day, "YYYY-MM-DD").format("MMM DD, YYYY"),
          day.summary.total - prev,
        ];
        let objData = [
          moment(day.day, "YYYY-MM-DD").format("MMM DD, YYYY"),
          day.summary.discharged - prevRecov,
        ];
        ...
        if (i == response["data"]["data"].length - 1) {
          res
            .status(200)
            .json({ activeData: activedata, recoveryData: recoveryData });
        }
      });
    })
...
router.post("/reserve", verify, async (req, res) => {
  try {
    ...
    const { type, pdfLink, name, hospitalId, covidTrace, aadharCard, age } =
      req.body;
...
router.get("/:bookingId", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: 1, data: bookingDetail, message: "Data Fetched" });
  } catch (err) {
